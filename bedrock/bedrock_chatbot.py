...
def configure_aws():
    ...
    try:
        ...
        st.error(f"Unexpected error during AWS configuration: {str(e)}")
        st.stop()
...
def render_sidebar() -> Tuple[Dict, str, str]:
    """
    ...
    with st.sidebar:
        ...
        # Add Google Docs save button and saved links area
        st.markdown("### Save Chat History")
        if st.button("Save to Google Docs"):
            if "messages" in st.session_state and len(st.session_state.messages) > 1:  # More than just the init message
                success, doc_url = save_to_google_docs(st.session_state.messages)
                if success:
                    if "saved_docs" not in st.session_state:
                        st.session_state.saved_docs = []
                    st.session_state.saved_docs.append(doc_url)
                    st.success("Successfully saved to Google Docs!")
                else:
                    st.error(f"Failed to save: {doc_url}")
            else:
                st.warning("No chat messages to save yet!")
        
        # Display saved document links
        if "saved_docs" in st.session_state and st.session_state.saved_docs:
            st.markdown("### Saved Documents")
            for doc_url in st.session_state.saved_docs:
                st.markdown(f"[Chat Document]({doc_url})", unsafe_allow_html=True)
...
def format_messages_for_docs(messages):
    """Format chat messages for Google Docs."""
    formatted_content = []
    for msg in messages:
        ...
        if isinstance(content, str):
            ...
            message_content = content.get("response", str(content))
        ...
        if "images" in msg and msg["images"]:
            message_content += f'\n[Attached Images: {len(msg["images"])} images]'
            
        formatted_content.append(f"{role}: {message_content}\n\n")
    
    return "".join(formatted_content)

def save_to_google_docs(messages):
    """Save chat history to Google Docs and return the document URL."""
    import requests
    from datetime import datetime
    
    try:
        formatted_content = format_messages_for_docs(messages)
        
        # Google Apps Script webhook URL
        GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbyDOjcHw3tgtS8Ng2y8vK1gV0QIlQBMLgqRi1tqZU0IzXuVgLu0FNb_JMn4EinsnMsx/exec"
        
        # Prepare the payload
        payload = {
            "content": formatted_content,
            "title": f"Bedrock Chat - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        }
        ...
        if response.status_code == 200:
            data = response.json()
            return True, data.get('documentUrl', '')
        else:
            return False, f"Failed to save: HTTP {response.status_code}"
...
def main() -> None:
    ...
    try:
        ...
        st.error(f"Failed to configure AWS: {str(e)}")
        st.stop()  # Stop execution if AWS configuration fails
    ...
    # Initialize ChatModel with bedrock_runtime
